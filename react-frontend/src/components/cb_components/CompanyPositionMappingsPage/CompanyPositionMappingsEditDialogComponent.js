...
const CompanyPositionMappingsEditDialogComponent = (props) => {
  ...
  useEffect(() => {
    if (props.entity) {
      set_entity({
        ...props.entity,
        position: props.entity.position?.map((pos) => pos._id) || [],
      });
      // Set mappingType based on props.entity if available, otherwise default to 'include'
      setMappingType(props.entity.mappingType || "include");
    }
  ...
  const onSave = async () => {
    ...
    try {
      ...
      props.alert({
        type: "success",
        title: "Edit Company Position Mapping",
        message: "Company Position Mapping updated successfully",
      });
      ...
      props.alert({
        type: "error",
        title: "Edit Company Position Mapping",
        message: "Failed to update Company Position Mapping",
      });
    }
    setLoading(false);
  };
  ...
  return (
    <Dialog
      header="Edit Company Position Mapping"
      visible={props.show}
      closable={false}
      onHide={props.onHide}
      modal
    ...
    >
      <div
        ...
        style={{ maxWidth: "55vw" }}
        role="companyPositionMappings-edit-dialog-component"
      >
