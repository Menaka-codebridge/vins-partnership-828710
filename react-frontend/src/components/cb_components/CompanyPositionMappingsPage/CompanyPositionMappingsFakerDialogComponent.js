...
import companyPositionMappingsFakerFactory from "./companyPositionMappingsFakerFactory";
...
const CompanyPositionMappingsFakerDialogComponent = (props) => {
  ...
  const onRun = async () => {
    let fakeData = companyPositionMappingsFakerFactory(
      props.user,
      fakerCount,
      companyIds,
      positionsIds,
    ...
    const promises = fakeData.map((elem) =>
      client.service("companyPositionMappings").create(elem),
    );
