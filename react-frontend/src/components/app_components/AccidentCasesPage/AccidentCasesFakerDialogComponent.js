...
import accidentCasesFakerFactory from "./accidentCasesFakerFactory";
...
const AccidentCasesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = accidentCasesFakerFactory(props.user,fakerCount,userIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("accidentCases").create(elem));
...
export default connect(mapState, mapDispatch)(AccidentCasesFakerDialogComponent);
