...
const AccidentCasesCreateDialogComponent = (props) => {
  ...
  const validate = () => {
    ...
    if (_.isEmpty(_entity?.summonsNo)) {
      error["summonsNo"] = `Case No field is required`;
      ret = false;
    }
  ...
  const onSave = async () => {
    ...
    try {
      ...
      props.alert({
        type: "success",
        title: "Create info",
        message: "Info Accident Cases updated successfully",
      });
      ...
      props.alert({
        type: "error",
        title: "Create",
        message: "Failed to create in Accident Cases",
      });
    }
    setLoading(false);
  };
  ...
  return (
    ...
    >
      <div
        ...
        style={{ maxWidth: "55vw" }}
        role="accidentCases-create-dialog-component"
      >
