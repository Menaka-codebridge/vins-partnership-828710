...
import sectionContentsFakerFactory from "./sectionContentsFakerFactory";
...
const SectionContentsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = sectionContentsFakerFactory(props.user,fakerCount,caseNoIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("sectionContents").create(elem));
...
export default connect(mapState, mapDispatch)(SectionContentsFakerDialogComponent);
