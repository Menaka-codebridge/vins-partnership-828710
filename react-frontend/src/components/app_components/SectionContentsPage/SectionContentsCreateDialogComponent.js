...
const SectionContentsCreateDialogComponent = (props) => {
  ...
  const onSave = async () => {
    ...
    try {
      ...
      const eagerResult = await client.service("sectionContents").find({
        query: {
          $limit: 10000,
          _id: { $in: [result._id] },
          $populate: [
      ...
      props.alert({
        type: "success",
        title: "Create info",
        message: "Info Section Contents updated successfully",
      });
      ...
      props.alert({
        type: "error",
        title: "Create",
        message: "Failed to create in Section Contents",
      });
    }
    setLoading(false);
  };
  ...
  return (
    <Dialog
      header="Create Section Contents"
      visible={props.show}
      closable={false}
      onHide={props.onHide}
      modal
    ...
    >
      <div
        ...
        style={{ maxWidth: "55vw" }}
        role="sectionContents-create-dialog-component"
      >
        ...
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <InputText
              id="initialInference"
              className="w-full mb-3 p-inputtext-sm"
              value={_entity?.initialInference}
              onChange={(e) => setValByKey("initialInference", e.target.value)}
              required
            />
          </span>
          <small className="p-error">
        ...
        <div className="col-12 md:col-6 field">
          <span className="align-items-center">
            ...
            <InputText
              id="groundTruth"
              className="w-full mb-3 p-inputtext-sm"
              value={_entity?.groundTruth}
              onChange={(e) => setValByKey("groundTruth", e.target.value)}
              required
            />
          </span>
          <small className="p-error">
