...
import CaseInteractionPage from "./CaseInteractionPage";
...
const SectionContentsPage = (props) => {
  ...
  const filename = "sectionContents";
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          summonsNo: urlParams.singleAccidentCasesId,
          $populate: [
            {
              path: "createdBy",
              service: "users",
      ...
      .catch((error) => {
        ...
        props.alert({
          ...
          type: "error",
          message: error.message || "Failed get Section Contents",
        });
      });
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("sectionContents").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
