...
import SectionContentsDatatable from "./SectionContentsDataTable";
import SectionContentsEditDialogComponent from "./SectionContentsEditDialogComponent";
import SectionContentsCreateDialogComponent from "./SectionContentsCreateDialogComponent";
import SectionContentsFakerDialogComponent from "./SectionContentsFakerDialogComponent";
import SectionContentsSeederDialogComponent from "./SectionContentsSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const SectionContentsPage = (props) => {
    ...
    const filename = "sectionContents.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("sectionContents");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , caseNo : urlParams.singleAccidentCasesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Section Contents", type: "error", message: error.message || "Failed get Section Contents" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("sectionContents").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Section Contents", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("sectionContents").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/sectionContents/${data._id}`);
    };
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="sectionContents-add-button"/>
            ...
            <div className="grid align-items-center">
                <div className="col-11" role="sectionContents-datatable">
                    <SectionContentsDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <SectionContentsEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <SectionContentsCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <SectionContentsFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <SectionContentsSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(SectionContentsPage);
