...
import TextExtractionQueuesDatatable from "./TextExtractionQueuesDataTable";
...
import TextExtractionQueuesFakerDialogComponent from "./TextExtractionQueuesFakerDialogComponent";
import TextExtractionQueuesSeederDialogComponent from "./TextExtractionQueuesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const TextExtractionQueuesPage = (props) => {
    ...
    const filename = "textExtractionQueues.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("textExtractionQueues");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , caseDocumentId : urlParams.singleCaseDocumentsId ,documentStorageId : urlParams.singleCaseDocumentsId ,caseNo : urlParams.singleAccidentCasesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
                ...
                select: ["name"],
                        ...
                        path : "caseDocumentId",
                        service : "case_documents",
                        select:["caseNo"]
                    },{
                        path : "documentStorageId",
                        service : "case_documents",
                        select:["caseNo"]
                    },{
                        path : "caseNo",
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "Text Extraction Queues", type: "error", message: error.message || "Failed get Text Extraction Queues" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("textExtractionQueues").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "Text Extraction Queues", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("textExtractionQueues").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
                title: "Text Extraction Queues",
                type: "error",
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/textExtractionQueues/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>Text Extraction Queues </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="textExtractionQueues-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="textExtractionQueues-datatable">
                    <TextExtractionQueuesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <TextExtractionQueuesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <TextExtractionQueuesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <TextExtractionQueuesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <TextExtractionQueuesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(TextExtractionQueuesPage);
