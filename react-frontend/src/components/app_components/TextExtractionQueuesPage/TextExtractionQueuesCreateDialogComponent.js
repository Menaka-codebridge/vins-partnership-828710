...
const [caseNo, setCaseNo] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [caseDocumentId,documentStorageId,caseNo], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            caseDocumentId: _entity?.caseDocumentId?._id,documentStorageId: _entity?.documentStorageId?._id,documentType: _entity?.documentType,caseNo: _entity?.caseNo?._id,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Text Extraction Queues updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Text Extraction Queues" });
        }
        setLoading(false);
    };
...
const caseNoOptions = caseNo.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Text Extraction Queues" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="textExtractionQueues-create-dialog-component">
