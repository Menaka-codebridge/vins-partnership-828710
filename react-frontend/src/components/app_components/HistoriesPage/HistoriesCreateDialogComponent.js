...
const [users, setUsers] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [caseNo,users], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            caseNo: _entity?.caseNo?._id,users: _entity?.users?._id,timestamp: _entity?.timestamp,userPrompt: _entity?.userPrompt,parametersUsed: _entity?.parametersUsed,synonymsUsed: _entity?.synonymsUsed,responseReceived: _entity?.responseReceived,section: _entity?.section,subSection: _entity?.subSection,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Histories updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Histories" });
        }
        setLoading(false);
    };
...
const usersOptions = users.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create Histories" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="histories-create-dialog-component">
