...
import historiesFakerFactory from "./historiesFakerFactory";
...
const HistoriesFakerDialogComponent = (props) => {
...
client.service("users")
    ...
    const onRun = async () => {
        let fakeData = historiesFakerFactory(props.user,fakerCount,caseNoIds,usersIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("histories").create(elem));
...
export default connect(mapState, mapDispatch)(HistoriesFakerDialogComponent);
