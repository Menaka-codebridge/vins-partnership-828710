...
import TokenUsageDatatable from "./TokenUsageDataTable";
...
import TokenUsageFakerDialogComponent from "./TokenUsageFakerDialogComponent";
import TokenUsageSeederDialogComponent from "./TokenUsageSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const TokenUsagePage = (props) => {
  ...
  useEffect(() => {
    const _getSchema = async () => {
      const _schema = await props.getSchema("tokenUsage");
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          sectionContentId: urlParams.singleCompaniesId,
          promptQueueId: urlParams.singleCompaniesId,
          summonsNo: urlParams.singleUsersId,
          $populate: [
            {
              path: "createdBy",
              service: "users",
  ...
  const deleteRow = async () => {
    try {
      await client.service("tokenUsage").remove(selectedEntityIndex);
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("tokenUsage").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="tokenUsage-add-button"
            />
          </>
        </div>
      </div>
      <div className="grid align-items-center">
        <div className="col-11" role="tokenUsage-datatable">
          <TokenUsageDatatable
            items={data}
            fields={fields}
...
export default connect(mapState, mapDispatch)(TokenUsagePage);
