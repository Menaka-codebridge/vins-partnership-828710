...
const [caseNo, setCaseNo] = useState([])
    ...
    useEffect(() => {
        ...
        if (!_.isEmpty(props?.entity)) {
            init = initilization({ ...props?.entity, ...init }, [caseDocumentId,caseNo], setError);
        }
        set_entity({...init});
        setError({});
    }, [props.show]);
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            caseDocumentId: _entity?.caseDocumentId?._id,caseNo: _entity?.caseNo?._id,status: _entity?.status,errorMessage: _entity?.errorMessage,
        ...
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info GroundTruthQueues updated successfully" });
        props.onCreateResult(eagerResult.data[0]);
        } catch (error) {
            console.debug("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in GroundTruthQueues" });
        }
        setLoading(false);
    };
...
const caseNoOptions = caseNo.map((elem) => ({ name: elem.name, value: elem.value }));
    ...
    return (
        <Dialog header="Create GroundTruthQueues" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="groundTruthQueues-create-dialog-component">
...
export default connect(mapState, mapDispatch)(GroundTruthQueuesCreateDialogComponent);
