...
const GroundTruthQueuesPage = (props) => {
  ...
  const filename = "groundTruthQueues";
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("groundTruthQueues").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
        ...
        <div className="col-6 flex justify-content-end">
          <>
            ...
            <Button
              ...
              role="groundTruthQueues-add-button"
            />
          </>
        </div>
      </div>
