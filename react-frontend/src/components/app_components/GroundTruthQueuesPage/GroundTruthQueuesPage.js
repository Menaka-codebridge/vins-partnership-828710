...
import GroundTruthQueuesDatatable from "./GroundTruthQueuesDataTable";
import GroundTruthQueuesEditDialogComponent from "./GroundTruthQueuesEditDialogComponent";
import GroundTruthQueuesCreateDialogComponent from "./GroundTruthQueuesCreateDialogComponent";
import GroundTruthQueuesFakerDialogComponent from "./GroundTruthQueuesFakerDialogComponent";
import GroundTruthQueuesSeederDialogComponent from "./GroundTruthQueuesSeederDialogComponent";
import SortIcon from "../../../assets/media/Sort.png";
import FilterIcon from "../../../assets/media/Filter.png";

const GroundTruthQueuesPage = (props) => {
    ...
    const filename = "groundTruthQueues.csv";
    ...
    useEffect(() => {
        const _getSchema = async () => {
            const _schema = await props.getSchema("groundTruthQueues");
    ...
    useEffect(() => {
        ...
        client
            ...
            .find({ query: { $limit: 10000 , caseDocumentId : urlParams.singleCaseDocumentsId ,caseNo : urlParams.singleAccidentCasesId  , $populate : [
                {
                path: "createdBy",
                service: "users",
                select: ["name"],
            ...
            .catch((error) => {
                ...
                setLoading(false);
                props.hide();
                props.alert({ title: "GroundTruthQueues", type: "error", message: error.message || "Failed get GroundTruthQueues" });
            });
  ...
  const onClickSaveHiddenfields = (ff) => {
    ...
    const deleteRow = async () => {
        try {
            await client.service("groundTruthQueues").remove(selectedEntityIndex);
            ...
            console.log({ error });
            props.alert({ title: "GroundTruthQueues", type: "error", message: error.message || "Failed delete record" });
        }
    };
    ...
    const deleteAll = async () => {
      ...
      props.show();
        ...
        const promises = data.map((e) => client.service("groundTruthQueues").remove(e._id));
        await Promise.all(
          promises.map((p) =>
            p.catch((error) => {
              props.alert({
    ...
    const onRowClick = ({data}) => {
        ...
        navigate(`/groundTruthQueues/${data._id}`);
    };
            ...
            <div className="grid">
                <div className="col-6 flex align-items-center justify-content-start">
                    <h4 className="mb-0 ml-2">
                        <span> My App /{" "}</span>
                        <strong>GroundTruthQueues </strong>
                    </h4>
                    <SplitButton
...
rounded loading={loading} icon="pi pi-plus" onClick={() => setShowCreateDialog(true)} role="groundTruthQueues-add-button"/>
                    
                    </>
                </div>
            </div>
            <div className="grid align-items-center">
                <div className="col-11" role="groundTruthQueues-datatable">
                    <GroundTruthQueuesDatatable items={data} fields={fields} onRowDelete={onRowDelete} onEditRow={onEditRow} onRowClick={onRowClick} searchDialog={searchDialog} setSearchDialog={setSearchDialog}
...
/>            
      ...
      />
            ...
            <GroundTruthQueuesEditDialogComponent entity={_.find(data,{ _id : selectedEntityIndex})} show={showEditDialog} onHide={() => setShowEditDialog(false)} onEditResult={onEditResult} />
            <GroundTruthQueuesCreateDialogComponent entity={newRecord} onCreateResult={onCreateResult} show={showCreateDialog} onHide={() => setShowCreateDialog(false)}  />
            <GroundTruthQueuesFakerDialogComponent show={showFakerDialog} onHide={() => setShowFakerDialog(false)} onFakerCreateResults={onFakerCreateResults} />
            <GroundTruthQueuesSeederDialogComponent show={showSeederDialog} onHide={() => setShowSeederDialog(false)} onSeederResults={onSeederResults} />
...
export default connect(mapState, mapDispatch)(GroundTruthQueuesPage);
