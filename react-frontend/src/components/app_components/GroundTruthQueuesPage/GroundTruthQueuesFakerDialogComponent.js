...
import groundTruthQueuesFakerFactory from "./groundTruthQueuesFakerFactory";
...
const GroundTruthQueuesFakerDialogComponent = (props) => {
...
client.service("accidentCases")
    ...
    const onRun = async () => {
        let fakeData = groundTruthQueuesFakerFactory(props.user,fakerCount,caseDocumentIdIds,caseNoIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("groundTruthQueues").create(elem));
...
export default connect(mapState, mapDispatch)(GroundTruthQueuesFakerDialogComponent);
