...
import promptQueuesFakerFactory from "./promptQueuesFakerFactory";
...
const PromptQueuesFakerDialogComponent = (props) => {
    const [fakerCount, setFakerCount] = useState(1);
    const [errors, setErrors] = useState([]);
    const [loading, setLoading] = useState(false);
    const [sectionContentIdIds, setsectionContentIdIds] = useState();
const [summonsNoIds, setsummonsNoIds] = useState();


    useEffect(() => {
        setFakerCount(1);
       
        client.service("sectionContents")
                        .find({ query: { $select: ["_id","subsection"] } })
                        .then((data) => {
                        setsectionContentIdIds(data.data);
                        });
client.service("accidentCases")
                        .find({ query: { $select: ["_id","insuranceRef"] } })
                        .then((data) => {
                        setsummonsNoIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = promptQueuesFakerFactory(props.user,fakerCount,sectionContentIdIds,summonsNoIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("promptQueues").create(elem));
...
export default connect(mapState, mapDispatch)(PromptQueuesFakerDialogComponent);
