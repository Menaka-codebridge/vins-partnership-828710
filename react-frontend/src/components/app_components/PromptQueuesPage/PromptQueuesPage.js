...
const PromptQueuesPage = (props) => {
  ...
  useEffect(() => {
    ...
    client
      ...
      .find({
        query: {
          $limit: 10000,
          sectionContentId: urlParams.singleSectionContentsId,
      ...
      .catch((error) => {
        ...
        props.alert({
          title: "PromptQueues",
          type: "error",
          message: error.message || "Failed get PromptQueues",
        });
      });
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("promptQueues").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  return (
    <div className="mt-5">
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <PromptQueuesEditDialogComponent
