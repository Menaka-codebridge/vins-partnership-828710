...
const CaseDocumentsPage = (props) => {
   ...
   results = _.orderBy(results, ["createdAt"], ["desc"]);
      ...
      .catch((error) => {
        ...
        props.alert({
          ...
          type: "error",
          message: error.message || "Failed get Case Documents",
        });
      });
  ...
  const deleteAll = async () => {
    ...
    const promises = data.map((e) =>
      client.service("caseDocuments").remove(e._id),
    );
    await Promise.all(
      promises.map((p) =>
        p.catch((error) => {
  ...
  return (
    <div className="mt-5">
      ...
      <AreYouSureDialog
        ...
        onYes={() => deleteRow()}
      />
      <CaseDocumentsEditDialogComponent
