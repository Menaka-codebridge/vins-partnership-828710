...
import caseDocumentsFakerFactory from "./caseDocumentsFakerFactory";
...
const CaseDocumentsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = caseDocumentsFakerFactory(props.user,fakerCount,caseNoIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("caseDocuments").create(elem));
...
export default connect(mapState, mapDispatch)(CaseDocumentsFakerDialogComponent);
