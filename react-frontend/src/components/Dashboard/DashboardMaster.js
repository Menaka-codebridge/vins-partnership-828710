...
import LineChart from "./Charts/LineChart/MachineMasterLineChart";
import CompanyBarChart from "./Charts/BarChart/MachinemasterBarChart";
import MasterDataServices from "./TabView/seviceTables/MasterServices";
...
export const AdminControl = (props) => {
      ...
      try {
        ...
        if (selectedProfile?.preferences) {
          ...
          const filteredPinnedItems = favourites
            .filter((item) => item.mainMenu === "master")
            .slice(-3)
            .map((item) => ({
              text: item.label,
  ...
  useEffect(() => {
    const fetchTotalCompanies = async () => {
      try {
        const { total } = await client.service("vendingMachines").find({
          query: {
            $limit: 0,
          },
        });
        ...
        <div className="mb-2 flex justify-content-between align-items-center">
          <div className="mb-4 flex justify-content-between align-items-center">
            <span className="text-900 font-medium text-3xl m-0.5">
              Master Data {showDash ? " Dashboard" : ""}
            </span>
          </div>
        ...
        >
          <div className="grid">
            ...
            <div className="col-12 md:col-4">
              <TotalComponent
                TotalComp={"Total Vending Machines"}
                isEdit={isEdit}
        ...
        <div className={classNames("", { hidden: !showDash })}>
          ...
          <div className="grid">
            ...
            <div className="col-12 md:col-7 mb-3 relative">
              <LineChart name={"Total Machines"} isEdit={isEdit} />
            </div>
