const { Histories } = require("./histories.class");
const createModel = require("../../models/histories.model");
...
module.exports = function (app) {
  ...
  app.use("/histories", new Histories(options, app));

  // Get our initialized service so that we can register hooks
  const service = app.service("histories");

  // Get the schema of the collections 
  app.get("/historiesSchema", function (request, response) {
